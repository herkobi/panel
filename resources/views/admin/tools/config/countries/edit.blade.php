    ...
    <div class="page-content">
        <div class="container">
            <div class="row mb-5">
                ...
                <div class="col-lg-9">
                    <div class="page-form">
                        <h3 class="form-title border-bottom mb-3 pb-3">Ülke Bilgilerini Düzenle</h3>
                        <form action="{{ route('panel.tools.config.country.update', $country->id) }}" method="POST">
                            @csrf
                                        ...
                                        @foreach (Status::cases() as $type)
                                            <label class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="status"
                                                    value="{{ $type->value }}"
                                                    {{ $type->value == $country->status->value ? 'checked' : '' }}>
                            ...
                            <div class="row mb-3">
                                <label for="name" class="col-form-label col-lg-2 col-md-3">Ülke Adı</label>
                                <div class="col-lg-10 col-md-9">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="name" id="name"
                                            class="form-control rounded-0 border-start-0" placeholder="Ülke Adını Giriniz"
                                            value="{{ old('name', $country->name) }}" required>
                                    </div>
                                    <span class="form-hint small">Ülke adını giriniz</span>
                                    @error('name')
                                        <div class="invalid-feedback" role="alert">Lütfen ülke adını giriniz</div>
                                    @enderror
                                </div>
                            </div>
                            <div class="row mb-3">
                                <label for="code" class="col-form-label col-lg-2 col-md-3">Ülke Kodu</label>
                                <div class="col-lg-10 col-md-9">
                                    <div class="input-group">
                                        <div class="input-group-text rounded-0 border-end-0 bg-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                        ...
                                        <input type="text" name="code" id="code"
                                            class="form-control rounded-0 border-start-0" placeholder="Ülke Kodunu Giriniz"
                                            value="{{ old('code', $country->code) }}" required>
                                    </div>
                                    <span class="form-hint small">Ülke kodunu giriniz</span>
                                    @error('code')
                                        <div class="invalid-feedback" role="alert">Lütfen ülke kodunu giriniz</div>
                                    @enderror
                                </div>
                            </div>
                            <div class="row mb-3">
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('panel.tools.config.country.delete', $country->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="modal-body">
                        <strong>{{ $country->name }}</strong> isimli ülkeyi silmek üzeresiniz. Bu işlem geri
                    ...
                    <div class="modal-footer">
                        <div class="d-flex align-items-center justify-content-between w-100">
                            <button type="button" class="btn bg-white border-0 text-dark" data-bs-dismiss="modal">İptal
                                Et</button>
                            <button type="submit" class="btn">Evet, Ülkeyi Sil</button>
                        </div>
                    </div>
                </form>
            </div>
