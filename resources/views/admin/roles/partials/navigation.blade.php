<div class="dropdown-menu panel-dropdown shadow-none">
    ...
    @if (request()->routeIs(['panel.roles', 'panel.role.*']))
        <a class="dropdown-item {{ request()->routeIs('panel.role.create') ? 'active' : '' }} "
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="icon dropdown-item-icon" width="24" height="24"
                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-shield-plus">
                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                <path
    ...
    @if (request()->routeIs(['panel.permissions', 'panel.permission.*']))
        <div class="dropdown-divider my-0"></div>
    @endif
    <a class="dropdown-item {{ request()->routeIs(['panel.permissions', 'panel.permission.edit']) ? 'active' : '' }}"
        href="{{ route('panel.permissions') }}" title="İzinler">
        <svg xmlns="http://www.w3.org/2000/svg" class="icon dropdown-item-icon" width="24" height="24"
            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
